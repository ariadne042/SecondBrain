...
- **Next step:** he restarts now (~07:4x) to load the keybinds, then tests `/digest` (two staged raws already waiting in `_system/digest-think/raw/`: the 09-20 crash session + the Factorio session). After the test: **the future-of-the-brain-and-Ariadne planning conversation** (his explicit ask this morning). Still pending IRL (unchanged): lid.conf real content + reboot, Steam first-run extraction finish, Factorio.
